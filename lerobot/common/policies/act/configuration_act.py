...
class ACTConfig:
    ...
    # 이미지와 6개의 모터 각도 입력 형태
    input_shapes: dict[str, list[int]] = field(
        default_factory=lambda: {
            "observation.images": [3, 480, 640],  # 3채널, 480x640 크기의 이미지
            "observation.state": [6],  # 6개의 모터 각도 (positions.npy)
        }
    )
    output_shapes: dict[str, list[int]] = field(
        default_factory=lambda: {
            "action": [6],  # 출력 20 time step에서의 6개의 모터 각도
        }
    )

    input_normalization_modes: dict[str, str] = field(
        default_factory=lambda: {
            "observation.images": "min_max",
