...
class RoboticDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        action_is_pad = torch.ones(self.output_steps * 6, dtype=torch.float32)

        return {
            "observation.images": images_input,  # (3, 480, 640) 형태로 반환
            "observation.state": torch.tensor(positions_input, dtype=torch.float32),  # (6)
            "action": torch.tensor(positions_output_flat, dtype=torch.float32),  # (output_steps * 6) 형태로 반환
            "action_is_pad": action_is_pad  # 패딩 여부
        }
